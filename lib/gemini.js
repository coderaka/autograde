...
export const MODELS = {
  'gemini-3.5-flash': { label: 'Gemini 3.5 Flash', tier: 'fast', provider: 'gemini', model: 'gemini-3.5-flash' },
  'gemini-3.1-pro-preview': { label: 'Gemini 3.1 Pro (Preview)', tier: 'pro', provider: 'gemini', model: 'gemini-3.1-pro-preview' },
  'codex-gpt-5.5-xhigh': { label: 'Codex CLI - GPT-5.5 (xhigh)', tier: 'cli', provider: 'codex-cli', model: 'gpt-5.5', reasoningEffort: 'xhigh' },
  'agy-gemini-3.1-pro-high': { label: 'AGY CLI - Gemini 3.1 Pro (High)', tier: 'cli', provider: 'agy-cli', model: 'Gemini 3.1 Pro (High)' },
  'agy-gemini-3.5-flash-high': { label: 'AGY CLI - Gemini 3.5 Flash (High)', tier: 'cli', provider: 'agy-cli', model: 'Gemini 3.5 Flash (High)' },
};
...
export function getModelConfig(model) { return MODELS[model] || null; }
export function isCliModel(model) {
  const info = getModelConfig(model);
  return info?.provider === 'codex-cli' || info?.provider === 'agy-cli';
}
export function resolveGeminiModel(model) {
  const key = model || currentModel;
  const info = getModelConfig(key);
  if (!info) return key;
  return info.provider === 'gemini' ? (info.model || key) : DEFAULT_MODEL;
}
...
export async function gradePdfWithSchema(pdfPath, systemPrompt, userPrompt, jsonSchema, model, { onRetry } = {}) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const isRetryable = isJsonParseError
